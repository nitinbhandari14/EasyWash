...
public class OrderhController {
	...
	@GetMapping("/ordercreate/get/{cid}")
	public ModelAndView showSingleUser(@PathVariable("cid") Long cid, Model model) throws IllegalArgumentException, Exception {
		...
		list.add(cid);
		Optional<Customer> ccc = userRepository.findById(cid);
		if(!ccc.isPresent())
		{
			modelAndView.addObject(EWConstants.MODEL_MESSAGE_ORDER, "No CID:"+cid+", present in the System.");		
			modelAndView.addObject(EWConstants.MODEL_ALERT_CLASS, EWConstants.MODEL_ALERT_FAIL);
			modelAndView.setViewName("manage");
			return modelAndView;
		
		}
		List<Easywashpackage> packCalc=packageRepository.findByPname(ccc.get().getSubscribedpackage());
		Easywashpackage pack =packCalc.get(0);
		pack.getLoadkg();pack.getTotalpiece();
		
		modelAndView.addObject("daysLeft",DateTimeUtil.diffInDaysWithFormat(pack.getValiddays(),DateTimeUtil.timeDiffFromCreateTSWRTCreateORModf(ccc.get().getCreatetts())));//-Integer.parseInt(pack.getValiddays()));	
		
		//logic to calculate leftover weight
		List<Orderh> listOrder=orderRepository.findBycref(String.valueOf(cid));
		float previousWeight=0;
		if(!Strings.isNullOrEmpty(ccc.get().getLname()))
		 previousWeight=Float.parseFloat(ccc.get().getLname());
		
		float packWeight=Float.parseFloat(pack.getLoadkg());
		...
		modelAndView.addObject(EWConstants.MODEL_MESSAGE_ORDER, "With CID:"+cid+",You are Creating "+(listExistingOrder.size()+1)+"st Order.");		
	...
	@GetMapping("/ordercreate/get")
	public RedirectView showConsolePageOrder(Model model, RedirectAttributes atts) {
	...
	@ResponseBody
	@PostMapping("/ordercreate/addOrder")
	...
	@GetMapping("/order/delete/{Idno}")
	public RedirectView deleteUser(@PathVariable("Idno") long id, Model model, RedirectAttributes atts) {
		Orderh order = orderRepository.findById(id)
				.orElseThrow(() -> new IllegalArgumentException("Invalid Order Id:" + id));
		orderRepository.delete(order);
		atts.addFlashAttribute(EWConstants.MODEL_ALERT_CLASS, EWConstants.MODEL_ALERT_SUCCESS);
		atts.addFlashAttribute(EWConstants.MODEL_SEARCH_ORDER_COUNT, "Sucessfully Deleted OrderID :"+id);

		return new RedirectView("/order/1");
	}
	@GetMapping("/order/getCID/{cid}")
